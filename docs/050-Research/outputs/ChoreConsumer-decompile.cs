...
public class ChoreConsumer : KMonoBehaviour, IPersonalPriorityManager
{
	...
	private static Func<object, ChoreConsumer, Util.IterationInstruction> FindNextChoreEvaluateEntryHelper = delegate(object obj, ChoreConsumer consumer) {
		...
		if (consumer.consumerState.solidTransferArm.IsCellReachable (cell)) {
			fetchChore.CollectChoresFromGlobalChoreProvider (consumer.consumerState, consumer.preconditionSnapshot.succeededContexts, consumer.preconditionSnapshot.failedContexts, is_attempting_override: false);
		}
		return Util.IterationInstruction.Continue;
	};
	...
	public PreconditionSnapshot GetLastSuccessfulPreconditionSnapshot ()
	{
		return lastSuccessfulPreconditionSnapshot;
	}
	...
	public bool FindNextChore (ref Chore.Precondition.Context out_context)
	{
		...
		if (consumerState.hasSolidTransferArm) {
			...
			GameScenePartitioner.Instance.VisitEntries (extents.x, extents.y, extents.width, extents.height, GameScenePartitioner.Instance.fetchChoreLayer, FindNextChoreEvaluateEntryHelper, this);
		} else {
	...
	public void AddUrge (Urge urge)
	{
		...
		urges.Add (urge);
		Trigger (-736698276, (object)urge);
	}

	public void RemoveUrge (Urge urge)
	{
		urges.Remove (urge);
		Trigger (231622047, (object)urge);
	}
