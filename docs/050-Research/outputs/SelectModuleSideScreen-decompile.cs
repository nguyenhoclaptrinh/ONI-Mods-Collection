...
public class SelectModuleSideScreen : KScreen
{
	...
	{
		if (facadeSelectionPanel == null)
		{
			facadeSelectionPanel = Util.KInstantiateUI<FacadeSelectionPanel>(facadeSelectionPanelPrefab, base.gameObject, force_active: true);
			facadeSelectionPanel.transform.SetSiblingIndex(materialSelectionPanel.transform.GetSiblingIndex());
		}
		facadeSelectionPanel.SetBuildingDef(selectedModuleDef.PrefabID);
	}
	...
	private bool TestBuildable(BuildingDef def)
	{
		...
		if (selectionContext == SelectModuleCondition.SelectionContext.AddModuleAbove && module != null)
		{
			...
			if (component != null && component.points[0].attachedBuilding != null && component.points[0].attachedBuilding.HasTag(GameTags.RocketModule) && !component.points[0].attachedBuilding.GetComponent<ReorderableBuilding>().CanMoveVertically(def.HeightInCells))
			{
				return false;
			}
		}
	...
	private void OrderBuildSelectedModule()
	{
		...
		if (gameObject != null)
		{
			...
			if (facadeSelectionPanel.SelectedFacade != null && facadeSelectionPanel.SelectedFacade != "DEFAULT_FACADE")
			{
				gameObject.GetComponent<BuildingFacade>().ApplyBuildingFacade(Db.GetBuildingFacades().Get(facadeSelectionPanel.SelectedFacade));
			}
