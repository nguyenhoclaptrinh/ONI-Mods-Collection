...
public class LaunchPad : KMonoBehaviour, ISim1000ms, IListableOption, IProcessConditionSet
{
	...
	protected override void OnSpawn()
	{
		...
		if (num < 35)
		{
			selectable.AddStatusItem(Db.Get().BuildingStatusItems.RocketPlatformCloseToCeiling, num);
		}
	}
	...
	private void CheckLandedRocketPassengerModuleStatus()
	{
		...
		{
			selectable.RemoveStatusItem(landedRocketPassengerModuleStatusItem);
		...
		else if (LandedRocket.CraftInterface.GetPassengerModule() == null)
		{
			...
			{
				landedRocketPassengerModuleStatusItem = selectable.AddStatusItem(Db.Get().BuildingStatusItems.LandedRocketLacksPassengerModule);
			}
		}
	...
	{
		int portCell = ports.GetPortCell(triggerPort);
	...
	public void Sim1000ms(float dt)
	{
		...
		{
			if (ports.GetInputValue(triggerPort) == 1)
			{
		...
		CheckLandedRocketPassengerModuleStatus();
		ports.SendSignal(landedRocketPort, (landedRocket != null) ? 1 : 0);
		if (landedRocket != null)
		{
			ports.SendSignal(statusPort, (landedRocket.CraftInterface.CheckReadyForAutomatedLaunch() || landedRocket.CraftInterface.HasTag(GameTags.RocketNotOnGround)) ? 1 : 0);
		}
		else
		{
			ports.SendSignal(statusPort, 0);
		}
	}
	...
	private void OnRocketBuildingChanged(object data)
	{
		...
		if (clustercraft != null)
		{
			...
			{
				Trigger(-887025858, (object)landedRocket);
			}
			else if (clustercraft.Status == Clustercraft.CraftStatus.Launching)
			{
				Trigger(-1277991738, (object)landedRocket);
