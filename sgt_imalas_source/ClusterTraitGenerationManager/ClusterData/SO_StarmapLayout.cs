...
namespace ClusterTraitGenerationManager.UI.SO_StarmapEditor
{
	public class SO_StarmapLayout
	{
		...
		/// <summary>
		/// Copied from the ProcGenGame.Cluster Class, AssignClusterLocations
		/// </summary>
		/// 


		public static AxialI BeachedBeltOrigin(int seed, int rings)
		{
		...
		/// <summary>
		/// Hexes that are considered "inside the belt
		/// </summary>
		HashSet<AxialI> BeltLocations;
		/// <summary>
		/// Hexes that are on the starter asteroid side of the belt
		/// </summary>
		HashSet<AxialI> InnerLocations;
		/// <summary>
		/// Hexes that are beyond the belt from the starter asteroid
		/// </summary>
		HashSet<AxialI> OuterLocations;
		public void Beached_InitializeAsteroidBelt(int seed)
		{
			BeltLocations = new();
			InnerLocations = new();
			OuterLocations = new();

			int rings = CGSMClusterManager.CustomCluster.Rings;
			AxialI beltSource = BeachedBeltOrigin(seed, rings);
			AxialI center = new(0, 0);

			//optional origin marker
			//OverridePlacements[beltSource] = "HarvestableSpacePOI_CarbonAsteroidField";

			var beltOriginPos = AxialUtil.AxialToWorld(beltSource.R, beltSource.Q);
			var centerPos = AxialUtil.AxialToWorld(0, 0);
			float worldRadius = Vector3.Distance(centerPos, beltOriginPos); //radius in world coordinates
			...
			foreach (AxialI location in AxialUtil.GetAllPointsWithinRadius(center, rings))
			{
				...
				var distanceF = Vector3.Distance(beltOriginPos, locationPoint);
				...
				BeltLocations.Add(location);
				OverridePlacements[location] = "HarvestableSpacePOI_RockyAsteroidField";
			}
		}
