...
| **Move Geyser Instant** | Di chuyển mạch phun/vent/volcano tức thời bằng nút trong detail, có overlay vị trí đặt và hỗ trợ đặt xuyên hành tinh. Tác giả: **nguyenhoclaptrinh**. | [`_source/MoveGeyserInstant`](./_source/MoveGeyserInstant) |
...
## 🧹 Thu gọn Workspace Local

Workspace có thể được thu gọn tại chỗ mà vẫn giữ lịch sử Git và các dependency cần thiết để build những mod đang bảo trì. Script mặc định chỉ báo cáo; thêm `-Apply` khi muốn thực sự dọn:

```powershell
.\tools\Compact-LocalWorkspace.ps1
.\tools\Compact-LocalWorkspace.ps1 -Apply
```

Script xóa cache `bin/`, `obj/`, `.vs/`, log và file debug runtime; giữ `archived_versions`, bản dịch `.mo`, cấu hình `Directory.Build.props.user`, source `lib` ignored và các output PLib cần cho AutoDropBottlers. Các file giữ lại được backup tạm qua bước sparse checkout. Script cũng chỉ giữ source đang bảo trì trong working tree và hardlink các dependency DLL trùng byte trên NTFS.

Khôi phục toàn bộ source tracked khi cần:

```powershell
.\tools\Compact-LocalWorkspace.ps1 -RestoreFullSource -Apply
```

Có thể bỏ qua từng bước tối ưu bằng `-SkipHardlinks` hoặc `-SkipSparseCheckout`.
